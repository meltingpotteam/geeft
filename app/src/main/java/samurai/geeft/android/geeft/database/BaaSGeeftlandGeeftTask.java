...
/**
 * Created by gabriel-dev on 28/02/16.
 */
...
public class BaaSGeeftlandGeeftTask extends AsyncTask<Void,Void,Boolean> {

    private static final String TAG ="BaaSGeeftlandTask";
    Context mContext;
    List<Geeft> mGeeftList;
    TaskCallbackBoolean mCallback;
    GeeftlandAdapter mGeeftlandAdapter;
    boolean result;

    public BaaSGeeftlandGeeftTask(Context context, List<Geeft> feedItems, GeeftlandAdapter Adapter,
                                   TaskCallbackBoolean callback) {
        mContext = context;
        mGeeftList = feedItems;
        mCallback = callback;
        mGeeftlandAdapter = Adapter;
        mGeeftList = feedItems;
    }
